...
class CSerialWrapper {
  public:
    enum ModeType { UNSET = 0, INITIALISING = 1, PACKETS = 2, RAWDATA = 4, BLOCKDATA = 8 };
    static const uint FRAMING_MASK = ModeType::PACKETS | ModeType::RAWDATA | ModeType::BLOCKDATA;
  ...
  protected:
    ...
    std::array<uint8_t, FRAMING_SIZE> m_Packet_Start  = { 0x0A, 0x50, 0x4B, 0x2B };
    std::array<uint8_t, FRAMING_SIZE> m_Packet_End    = { 0x0A, 0x50, 0x4B, 0x2D };
